...
def main():
    ...
    if args.height:
        ...
        model_D_path = osp.join(here,'results', DATASET + '_height_hrnet', 'checkpoints', 'model_D_iter' + str(args.resume) +'.pth')
    else:
        model_path = osp.join(here,'results', DATASET + '_no_height_hrnet', 'checkpoints', 'model_iter' + str(args.resume) +'.pth')
        model_D_path = osp.join(here,'results', DATASET + '_no_height_hrnet', 'checkpoints', 'model_D_iter' + str(args.resume) +'.pth')
    i_iter=0
    if args.resume>0:
    ...
    while i_iter < max_iter:
        ...
        pred_source = model(images)
        loss_seg = loss_fn(pred_source, labels)
        loss_seg.backward()

        loss_data = loss_seg.data.item()
        pred = np.argmax(pred_source.data.cpu().numpy()[0], axis=0)
