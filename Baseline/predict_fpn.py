...
def main():
    ...
    if args.height:
        out_dir = osp.join(here,'results', SOURCE + '_' + TARGET + '_height_fpn')
    else:
        out_dir = osp.join(here,'results', SOURCE + '_' + TARGET + '_no_height_fpn')
    ...
    for i in range(20):
        ...
        if args.height:
            path = osp.join(here,'results', SOURCE + '_' + TARGET + '_height_fpn', 'checkpoints', 'iter' + str(resume) +'.pth')
        else:
            path = osp.join(here,'results', SOURCE + '_' + TARGET + '_no_height_fpn', 'checkpoints', 'iter' + str(resume) +'.pth')
