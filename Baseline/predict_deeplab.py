...
def main():
    ...
    if args.height:
        out_dir = osp.join(here,'results', SOURCE + '_' + TARGET + '_height_deeplab')
    else:
        out_dir = osp.join(here,'results', SOURCE + '_' + TARGET + '_no_height_deeplab')
    ...
    for i in range(20):
        ...
        if args.height:
            path = osp.join(here,'results', SOURCE + '_' + TARGET + '_height_deeplab', 'checkpoints', 'iter' + str(resume) +'.pth')
        else:
            path = osp.join(here,'results', SOURCE + '_' + TARGET + '_no_height_deeplab', 'checkpoints', 'iter' + str(resume) +'.pth')
