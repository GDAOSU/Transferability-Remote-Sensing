    ...
    for i in range(len(SOURCE)):
        ...
        for j in range(len(TARGET)):
            ...
            for k in range(len(HEIGHT)):
                ...
                if height:
                    out_dir = osp.join(here,'results', source + '_' + source, source + '_' + source + '_height_deeplab')
                    input = 5
                else:
                    out_dir = osp.join(here,'results', source + '_' + source, source + '_' + source + '_no_height_deeplab')
                    input = 4
                ...
                for i in range(20):
                    ...
                    if height:
                        path = osp.join(here,'results', source + '_' + source, source + '_' + source + '_height_deeplab', 'checkpoints', 'iter'+str(resume)+'.pth')
                    else:
                        path = osp.join(here,'results', source + '_' + source, source + '_' + source + '_no_height_deeplab', 'checkpoints', 'iter'+str(resume)+'.pth')
