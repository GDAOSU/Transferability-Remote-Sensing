...
def main():
    ...
    if args.height:
        path = osp.join(here,'results', DATASET + '_height_deeplab', 'checkpoints', 'iter' + str(args.resume) +'.pth')
    else:
        path = osp.join(here,'results', DATASET + '_no_height_deeplab', 'checkpoints', 'iter' + str(args.resume) +'.pth')
    i_iter=0
    if args.resume>0:
