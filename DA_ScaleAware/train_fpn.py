...
def main():
    ...
    if args.height:
        ...
        netD_domain_path = osp.join(here,'results', DATASET + '_height_fpn', 'checkpoints', 'netD_domain_iter' + str(args.resume) +'.pth')
        netD_scale_path = osp.join(here,'results', DATASET + '_height_fpn', 'checkpoints', 'netD_scale_iter' + str(args.resume) +'.pth')
    else:
        model_path = osp.join(here,'results', DATASET + '_no_height_fpn', 'checkpoints', 'model_iter' + str(args.resume) +'.pth')
        netD_domain_path = osp.join(here,'results', DATASET + '_no_height_fpn', 'checkpoints', 'netD_domain_iter' + str(args.resume) +'.pth')
        netD_scale_path = osp.join(here,'results', DATASET + '_no_height_fpn', 'checkpoints', 'netD_scale_iter' + str(args.resume) +'.pth')
    i_iter=0
    if args.resume>0:
